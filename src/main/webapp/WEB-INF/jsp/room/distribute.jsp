		...
		<script type="text/javascript">
			...
			function deltearoom(ID) {
				if (confirm('确认删除?')) {
					$.ajax({
						type : "post",
						url : "./${sessionScope.pathCode}/delTeaRoom.do",
						data : "ID=" + ID,
						dataType : 'html',
			...
			function delAll() {
				if (confirm('确认全部删除?')) {
					$.ajax({
						type : "post",
						url : "./${sessionScope.pathCode}/delAllTeaRoom.do",
						dataType : 'html',
						contentType : "application/x-www-form-urlencoded; charset=utf-8",
						success : function(result) {
							location.reload();
			...
			function enter() {
				var url = "./${sessionScope.pathCode}/chooseTeacher.do";
				url = encodeURI(url);
						...
						<tbody>
							<c:forEach items="${tearooms}" var="tearoom">
								<tr>
									...
									<td><small>${tearoom.start} - ${tearoom.end}</small></td>
									<td>
										<button type="button" class="btn btn-danger" onclick="deltearoom('${tearoom.id}')">删除</button>
									</td>
								</tr>
							</c:forEach>
								<tr>
