...
public class AdminController implements ConfigDo{
	...
	public String resetStuPsw(@RequestParam("ID") Integer ID, Model model, HttpServletResponse response) throws Exception {
		serviceFit.getLoginService().updatePsw(ID, MDUtil.MD5Tools("123456"));
		return "redirect:./stuInfo.do";
	}
	
	@RequestMapping("resetTeaPsw")
	public String resetTeaPsw(@RequestParam("ID") Integer ID, Model model, HttpServletResponse response) throws Exception {
		serviceFit.getLoginService().updatePsw(ID, MDUtil.MD5Tools("123456"));
	...
	public String claInfo(Model model, HttpSession session) {
		ResultDo<?> resultDo = serviceFit.getAdminService().getClasses();
		List<?> classs = null;
		if (resultDo.isSuccess()) {
			classs = (List<?>) resultDo.getResult();
	...
	public String addClass(@RequestParam("type") boolean type, @RequestParam("grade") String grade, @RequestParam("classes_name") String name, Model model, HttpSession session) {
	...
	public String addStudent(@RequestParam("noid") String noid, @RequestParam("name") String name, @RequestParam("claname") String claname, @RequestParam("claid") String claid, Model model, HttpSession session) {
		List<Identity> identities = new ArrayList<>();
		Identity identity = new Identity();
		identity.setCla_id(Integer.parseInt(claid));
	...
	public String delStudent(@RequestParam("id") String id, Model model, HttpSession session) {
	...
	public String addTeacher(@RequestParam("noid") String noid, @RequestParam("name") String name, Model model, HttpSession session) {
		List<Identity> identities = new ArrayList<>();
		Identity identity = new Identity();
		identity.setNoid(noid);
		identity.setName(name);
	...
	public String proDistr(@RequestParam("proID") Integer proID, @RequestParam("teaID") Integer teaID, @RequestParam("teaName") String teaName, Model model, HttpSession session) {
		serviceFit.getAdminService().upDistr(proID, teaID, teaName);
		return "redirect:./projects.do";
	}
	
	@RequestMapping("proAggre")
	public String proAggre(@RequestParam("proID") Integer proID, @RequestParam("teaID") Integer teaID, @RequestParam("teaName") String teaName, Model model, HttpSession session) {
